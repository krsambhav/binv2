...
const BinChecker = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (bin.slice(0, 1) === "3" || bin.slice(0, 1) === "6") {
      setError("Only Visa/MasterCard Supported");
      setLoading(false);
      return;
    }
        ...
        <div className="mt-8 p-6 w-full max-w-lg bg-black rounded-lg shadow-lg backdrop-blur-md transition-all">
          ...
          <div className="space-y-2">
            ...
            <p className="text-lg text-white">
              <strong>Bank:</strong> {cardDetails.issuer}
            </p>
            <p className="text-lg text-white">
              <strong>Scheme:</strong> {cardDetails.scheme}
            </p>
            {/* <p className="text-lg text-white"><strong>Last 4:</strong> {cardDetails.last4}</p> */}
            <p className="text-lg text-white">
              <strong>Card Type:</strong> {cardDetails.card_type}
            </p>
            <p className="text-lg text-white">
              <strong>Level:</strong> {cardDetails.product_type}
            </p>
          </div>
        </div>
      )}
